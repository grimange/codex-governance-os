...
class TemplateCompositionPostServiceMonorepoProtectionsTests(unittest.TestCase):
    supported_cases = (
        [],
        ["laravel", "monorepo"],
        ["django", "monorepo"],
    ...
    def test_supported_matrix_remains_consistent_after_service_monorepo_expansion(self) -> None:
    ...
    def test_rejected_matrix_remains_fail_closed_after_service_monorepo_expansion(self) -> None:
    ...
    def test_contract_removal_drift_for_service_monorepo_is_detected(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            text = source.read_text().replace("- `service + monorepo`\n", "")
            ...
            self.assertIn(
                "CONTRACT_DRIFT_DETECTED: runtime supports undocumented composition monorepo + service",
                report.errors,
            )

    def test_runtime_expansion_drift_for_laravel_cli_worker_is_detected(self) -> None:
        ...
        self.assertFalse(report.valid)
        self.assertIn(
            "CONTRACT_DRIFT_DETECTED: runtime supports undocumented composition cli-worker + laravel",
            report.errors,
        )

    def test_manifest_drift_for_laravel_cli_worker_is_detected(self) -> None:
        repo = Path(__file__).resolve().parents[2]
        with tempfile.TemporaryDirectory() as tmp:
