...
- `docs/governance/session-reconstruction-verification-harness.md` defines how a claimed reconstructed session narrative is verified against Layer 6 evidence, precedence, determinism, and fail-closed invariants without reconstructing the session itself.
- `docs/governance/session-reconstruction-case-verification-model.md` defines the canonical structure of an individual reconstruction case so harness evaluation inputs remain bounded, cited, and restriction-aware across sessions.
- `docs/governance/session-reconstruction-evidence-packaging-standard.md` defines how evidence for reconstruction verification is packaged so case inputs remain ordered, explicitly cited, and restriction-preserving across sessions.
- `docs/governance/multi-session-continuity-verification-model.md` defines how continuity relationships between independently verified sessions are evaluated without merging session boundaries or replacing the single-session reconstruction stack.
- `docs/governance/multi-session-continuity-evidence-harness.md` defines the admissible evidence types, thresholds, procedure, and failure classes used to validate multi-session continuity without implicit cross-session reasoning.
- `docs/governance/multi-session-continuity-evaluation-scenarios.md` defines the canonical scenario fixtures used to exercise verified, weak, absent, and boundary-violating multi-session continuity conditions deterministically.
...
- Future reconstruction verification must follow `docs/governance/session-reconstruction-verification-harness.md` so governance observers classify reconstructed narratives through one explicit evidence-anchoring, precedence, determinism, and fail-closed validation model.
- Future reconstruction-case preparation must follow `docs/governance/session-reconstruction-case-verification-model.md` so harness evaluation inputs remain anchored on one `session_id`, explicitly scoped, fully cited, and restriction-preserving.
- Future reconstruction-evidence packaging must follow `docs/governance/session-reconstruction-evidence-packaging-standard.md` so packaged evidence remains deterministically ordered, explicitly cited, and visibly restricted across reconstruction cases.
- Future multi-session continuity verification must follow `docs/governance/multi-session-continuity-verification-model.md` so cross-session governance relationships remain evidence-backed, hierarchy-preserving, and boundary-safe without merging session truth.
- Future multi-session continuity evidence evaluation must follow `docs/governance/multi-session-continuity-evidence-harness.md` so cross-session linkage claims use explicit evidence types, bounded thresholds, deterministic continuity classifications, and strict session isolation.
